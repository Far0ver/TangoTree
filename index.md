...
##Содержание:
[Введение](#intro)
[Краткое описание строения дерева](#k)
[Вспомогательные деревья](#a_tree)
[Операции слияния и разделения во вспомогательных деревьях](#operations)
[Операция поиска в Tango дереве](#find)
[Splay дерево](#splay)
[Реализация](#upd)
[Источники](#info)
...
Дерево в изначальной теоретической реализации представляет собой созданные и объединенные по некоторому принципу вспомогательные деревья. Tango дерево поддерживает операцию поиска и операции разделения и слияния вспомогательных деревьев. Изначально в качестве вспомогательных деревьев использовались красно-черные, но последующие реализации, зачастую тоже теоретические, использовали уже модифицированные splay деревья, так как это было эффективнее по времени.
Для понимания строения tango дерева кратко рассмотрим что из себя представляет красно-черное дерево.

<a name="k"></a>
##Краткое описание строения дерева
...
<a name="operations"></a>
##Операции слияния и разделения во вспомогательных деревьях
...
<a name="splay"></a>
##Splay дерево
В этой реализации в качестве вспомогательных деревьев будут использоваться Splay деревья. Поэтому кратко опишем основные характеристики этого дерева и функции, которые будут ипользоваться.
...
![](src/4.png)
Дерево Tango на рисунке имеет либо сплошные, либо пунктирные линии для обозначения путей. Набор узлов, соединенных сплошными линиями, — вспомогательное Splay дерево. Узлы во вспомогательном дереве такие же, как и узлы в соответствующем предпочтительном пути в исходном дереве.
![](src/5.png)  
...
####Сложность
1. Амортизированное время, необходимое для доступа к элементам в Tango Tree, равно O(log n).
2. Временная сложность алгоритма O(log log n).
3. Для любого запроса в Tango Tree сложность в наихудшем случае составляет O(log²n). Это следует из того, что для поиска узла мы посещаем не более O(height) Splay деревьев. Поскольку размер каждого Splay дерева равен O(logn), то общее количество узлов, которые мы можем посетить, равно O(log²n).
